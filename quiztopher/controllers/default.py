...
#   print subCatNumber
   ...
   if db((db.counter.subcategory==session.subCatNumber) & (db.counter.idUser==auth.user.id)).count() == 0:
    #  print "hello"
      db.counter.insert(idUser=auth.user.id, subcategory=session.subCatNumber, startptr=0)      
  

   var = db((db.questions.RollNo == session.quesNo)).select()
   # & (db.questions.flag1 == db.counter.startptr) & (db.counter.idUser == auth.user.id) & (db.counter.subcategory == session.subCatNumber)).select().first()
   print var;
   #var = db(db.questions.RollNo == session.quesNo).select()
...
#     print session.ansDict[var1]
       ...
       pass 
   session.notAttempted = 10 - len(session.ansDict)
...
#   print session.score
   rows = db((db.counter.idUser == auth.user.id) & (db.counter.subcategory == session.subCatNumber)).select().first()
   #print "rows:"
   #print rows
   #for row in rows:
   #   print row.idUser
   #print row;
   rows.update(startptr=1)
   print rows
   return dict()
