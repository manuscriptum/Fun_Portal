...
db.define_table(
    ...
    Field('image','upload',readable=False,requires=[]),
...
IS_NOT_EMPTY(error_message=auth.messages.is_empty)
custom_auth_table.password.requires = [IS_STRONG(min=5), CRYPT()]
import os
custom_auth_table.image.default = os.path.join(request.folder, 'static', 'images', 'user.jpg')
...
db.define_table(
    ...
    Field('answer',default='opt1')
    )
db.questions.answer.requires=IS_IN_SET(['opt1','opt2','opt3','opt4'])
...
db.define_table(
    'userScore',
    Field('userid',default=auth.user.id if auth.user else 0),
    Field('dateTime','datetime',default=request.now),
    Field('Correctans','integer',label='Correct answers'),
    Field('Wrongans','integer'),
    Field('Notattempted','integer'),
    Field('Score','integer')
  )
db.userScore.userid.readable=db.userScore.userid.writable=False;
#db.userScore.truncate()
